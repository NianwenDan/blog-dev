...
function Postviews($archive) {
	...
	echo $exist == 0 ? 'Nobody Viewed' : $exist.' Viewed';
}

function Breadcrumbs($archive) {
	$options = Helper::options();
	if (!empty($options->Breadcrumbs) && in_array('Pageshow', $options->Breadcrumbs)) {
		echo '<div class="breadcrumbs">'.PHP_EOL .'<a href="'.$options->siteUrl.'">Home</a> &raquo; '.$archive->title.PHP_EOL .'</div>'.PHP_EOL;
	}
}
...
function createCatalog($obj) {
	...
	$obj = preg_replace_callback('/<h([1-6])(.*?)>(.*?)<\/h\1>/i', function($obj) {
		...
		return '<h'.$obj[1].$obj[2].'>'.$obj[3].'<a class="cl-offset" name="cl-'.$catalog_count.'"></a>'.'</h'.$obj[1].'>';
	}, $obj);
...
function getCatalog() {
	...
	$index = '<div id="catalog-col">'.PHP_EOL .'<b>TOC</b>'.PHP_EOL .$index.'<script>function Catalogswith(){document.getElementById("catalog-col").classList.toggle("catalog");document.getElementById("catalog").classList.toggle("catalog")}</script>'.PHP_EOL .'</div>'.PHP_EOL;
	}
	return $index;
}
